...
function TodoForm({ addTodo }: { addTodo: (text: string) => void }) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <input type="text" className="input w-full border-2 border-gray-300 rounded-md p-2" value={value} onChange={(e) => setValue(e.target.value)} placeholder="Add todo" />
        </form>
    );
}
...
function App() {
    ...
    const completeTodo = (index: number) => {
        const newTodos = [...todos.slice(0, index), { text: todos[index].text, isCompleted: true }, ...todos.slice(index + 1)];
        setTodos(newTodos);
    };
        ...
        <div className="app h-screen flex flex-col bg-white gap-3 justify-center items-center">
            ...
            <div className=" flex flex-col gap-2  ">
                {todos.map((todo, index) => (
                    <Todo deleteTodo={deleteTodo} key={index} index={index} todo={todo} completeTodo={completeTodo} />
                ))}
            </div>
        </div>
    );
